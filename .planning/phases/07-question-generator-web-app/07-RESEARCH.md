...
1. **What is the default Ollama model?**
   ...
   - Recommendation: Default to `llama3.2` (fastest), document model switching in settings

2. **What batch size limit should be implemented?**
   - What we know: D-16 requires batch size limits
   - What's unclear: Exact number (5? 10? 20?)
   - Recommendation: Start with 10 questions per batch, monitor performance

3. **How should connection errors be displayed?**
   - What we know: D-17 requires all calls be client-side
   - What's unclear: UX for "Ollama not running" or "CORS not configured"
   - Recommendation: Clear error message with setup instructions link

## Environment Availability

| Dependency | Required By | Available | Version | Fallback |
|------------|------------|-----------|---------|----------|
| Node.js | Build tools | Required | 20.x+ | — |
| Ollama | AI inference | User-provided | Any | N/A — required for generation |
| pnpm | Monorepo | Required | 9.x | — |
| Turborepo | Monorepo build | Required | 2.x | — |

**Missing dependencies with no fallback:**
- Ollama: Must be installed and running by user. App will display error if not available.

**Missing dependencies with fallback:**
- None — all other dependencies are npm packages

## Validation Architecture

> Skip this section — `workflow.nyquist_validation` is explicitly `false` in config.json.

## Security Domain

> Required when `security_enforcement` is enabled (absent = enabled). Phase 7 has external dependencies (Ollama), so include this section.

### Applicable ASVS Categories

| ASVS Category | Applies | Standard Control |
|---------------|---------|------------------|
| V2 Authentication | No | No user accounts (D-17 static export) |
| V3 Session Management | No | LocalStorage only, no sessions |
| V4 Access Control | No | No server-side resources |
| V5 Input Validation | Yes | Zod schema validation, input sanitization |
...
|---------|--------|---------------------|
| Prompt Injection | Tampering | Input sanitization, structured prompts, topic length limits |
| Content Generation Abuse | Information Disclosure | Rate limiting (batch size), category validation |
| Hallucinated Facts | Tampering | 3-pass verification, human review queue, confidence scoring |
| CORS Exploitation | Tampering | `OLLAMA_ORIGINS` configuration, clear documentation |

### Input Validation (ASVS V5)

```typescript
// All user inputs must be validated before LLM processing
const MAX_TOPIC_LENGTH = 100;
const MAX_GUIDANCE_LENGTH = 500;

function validateTopicInput(topic: string): string {
  if (topic.length > MAX_TOPIC_LENGTH) {
    throw new Error('Topic exceeds maximum length');
  }
  // Strip control characters that could inject instructions
  return topic.replace(/[\x00-\x1F\x7F]/g, '').trim();
}

// Category must be one of the 6 valid values
function validateCategory(category: string): Category {
  if (!['blue', 'pink', 'yellow', 'purple', 'green', 'orange'].includes(category)) {
...
- [Vercel AI SDK docs - generateObject with Zod] - Structured output patterns
- [ollama-ai-provider-v2 npm] - Ollama integration with Vercel AI SDK [VERIFIED: npm registry]
- [Next.js docs - Static Exports] - `output: 'export'` configuration
- [@trivial-world/types package] - Existing Zod schemas [VERIFIED: packages/types/src/question-pack.ts]
- [CONTEXT.md D-01 through D-19] - Locked architectural decisions

### Secondary (MEDIUM confidence)
- [GitHub ollama/ollama Issue #300] - CORS configuration for browser clients
- [ML Journey - Ollama + React integration] - Client-side patterns
- [BBC/EBU News Integrity Study (October 2025)] - AI hallucination rates
- [Council Mode paper (April 2026)] - Multi-model consensus verification
- [OWASP Top 10 for LLM Applications 2025] - Prompt injection risks

### Tertiary (LOW confidence)
- [DEV Community - Streaming SSE with Next.js] - Client-side streaming patterns
- [Local AI Master - Ollama + Vercel AI SDK] - Integration examples

## Metadata

**Confidence breakdown:**
- Standard stack: HIGH - All packages verified against npm registry, official docs
- Architecture: HIGH - Based on locked decisions in CONTEXT.md, Next.js docs
- Pitfalls: HIGH - Research-backed patterns from PITFALLS.md and academic sources
- Fact-checking: HIGH - Multi-pass consensus from Council Mode and HUMBR papers

**Research date:** 2026-06-08
**Valid until:** 2026-09-08 (3 months - fast-moving AI SDK space)